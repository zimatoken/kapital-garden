    ...
    <div className="garden-year">
      ...
      <div className="garden-row">
        {months.map((m) => (
          <button
            key={m.label}
            className={`garden-cell garden-cell-${m.status} ${selected?.label === m.label ? 'garden-cell-selected' : ''}`}
            onClick={() => setSelected(selected?.label === m.label ? null : m)}
            title={`${MONTH_NAMES[m.month - 1]}: ${m.daysActive}/${m.daysTotal} дней`}
          >
            <span className="garden-cell-emoji">{STATUS_EMOJI[m.status]}</span>
            <span className="garden-cell-label">{MONTH_NAMES[m.month - 1]}</span>
          </button>
        ))}
      </div>
      ...
      {selected && (
        <div className="garden-details">
          <strong>{MONTH_NAMES[selected.month - 1]} {year}</strong>
          <span>
